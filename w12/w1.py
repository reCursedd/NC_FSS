...
def pg7():
    ...
    # Result without importing division from __future__ because of py3
    assert bool(5 / 2 == 2.5) == True
...
def pg28():
    ...
    assert len(list(list_filter(["1 2 3 peter poll and marry", "3 2 1 go", "This is cool 2"]))[2]) == 1
...
def pg31():
    ...
    # py3 returns an iterable and not a list
    assert list(zip_list)[0][1] == 'apple'
...
def pg33():
    ...
    def twoArgFunc(a, b): return a+b
    def multiply(a, b): return a*b
    def higherOrderFunc(highArgs):
        # print "Highargs: ", highArgs
        def lowerOrderFunc(*args, **kwargs):
            return 2* multiply(*args, **kwargs)
        #@todo: ask, when exactly lowerOrderFunc gets called and with what input. eg. hOF(1) doesn't workS?
        return lowerOrderFunc
    var = higherOrderFunc(twoArgFunc)
    assert var(1,2) == 4
